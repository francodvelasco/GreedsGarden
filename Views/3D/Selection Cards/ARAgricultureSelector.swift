...
class ARAgricultureSelector: ARPresentable {
    static func entity(originPosition: SIMD3<Float>) -> AnchorEntity {
        ...
        anchor.name = "Selector-Agriculture"
        
        let buttonWrapper = ARRealityKitSwiftUIView(anchorTitle: "Selector-Agriculture") {
            VStack {
                Text("Agriculture")
                    .fontWeight(.medium)
                Spacer()
        ...
        let baseRectangle = MeshResource.generateBox(width: 0.1, height: 0.07, depth: 0.01, cornerRadius: 0.02)
        ...
        baseRectangleEntity.name = "Selector-Agriculture"
        ...
        textEntity.name = "Selector-Agriculture-Title"
        textEntity.setPosition(textPosition, relativeTo: nil)
        
        let sampleAnchor = AnchorEntity(world: anchorPosition)
        do {
            let sampleModel = try ModelEntity.loadModel(named: "Agriculture_Level1", in: nil)
            ...
            sampleEntity?.name = "Selector-Agriculture"
    ...
    @MainActor static func addToView(arView: ARView, baseAnchor: AnchorEntity, relativePosition: SIMD3<Float>?) {
        ...
        selectorAnchor.name = "Selector-Agriculture"
        
        let buttonWrapper = ARRealityKitSwiftUIView(anchorTitle: "Selector-Agriculture-Background", desiredWidth: 0.15, desiredHeight: 0.105) {
            Text("Agriculture")
                .fontWeight(.medium)
                .padding(8)
                .background(Color.white)
        ...
        baseRectangleEntity.name = "Selector-Agriculture-Background"
        ...
        textEntity.name = "Selector-Agriculture-Text"
        textEntity.generateCollisionShapes(recursive: true)
        selectorAnchor.addChild(textEntity)*/
        
        guard let pathName = Bundle.main.path(forResource: "Agriculture_Level1", ofType: "usdz"),
        ...
        sampleEntity.name = "Selector-Agriculture-ModelSample"
        sampleEntity.generateCollisionShapes(recursive: true)
