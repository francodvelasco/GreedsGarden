...
class ARIndustrySelector: ARPresentable {
    static func entity(originPosition: SIMD3<Float>) -> AnchorEntity {
        ...
        baseRectangleEntity.name = "Selector-Industry-Base"
        ...
        textEntity.name = "Selector-Industry-Title"
        textEntity.setPosition(textPosition, relativeTo: nil)
        
        let sampleAnchor = AnchorEntity(world: anchorPosition)
        do {
            let sampleModel = try ModelEntity.loadModel(named: "Industry_Level1", in: nil)
            ...
            sampleEntity?.name = "Selector-Industry-Sample"
    ...
    @MainActor static func addToView(arView: ARView, baseAnchor: AnchorEntity, relativePosition: SIMD3<Float>?) {
        ...
        selectorAnchor.name = "Selector-Industry"
        
        let buttonWrapper = ARRealityKitSwiftUIView(anchorTitle: "Selector-Industry-Background", desiredWidth: 0.15, desiredHeight: 0.105) {
            Text("Industry")
                .fontWeight(.medium)
                .padding(8)
        ...
        baseRectangleEntity.name = "Selector-Industry-Background"
        ...
        textEntity.name = "Selector-Industry-Text"
        selectorAnchor.addChild(textEntity)*/
        
        guard let pathName = Bundle.main.path(forResource: "Industry_Level1", ofType: "usdz"),
        ...
        sampleEntity.name = "Selector-Industry-ModelSample"
