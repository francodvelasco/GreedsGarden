...
class ARTreeSelector: ARPresentable {
    static func entity(originPosition: SIMD3<Float>) -> AnchorEntity {
        ...
        baseRectangleEntity.name = "Selector-Tree-Base"
        ...
        textEntity.name = "Selector-Tree-Title"
        textEntity.setPosition(textPosition, relativeTo: nil)
        
        let sampleAnchor = AnchorEntity(world: anchorPosition)
        do {
            let sampleModel = try ModelEntity.loadModel(named: "Tree_Stage0_Level1", in: nil)
            ...
            sampleEntity?.name = "Selector-Tree-Sample"
    ...
    @MainActor static func addToView(arView: ARView, baseAnchor: AnchorEntity, relativePosition: SIMD3<Float>?) {
        ...
        selectorAnchor.name = "Selector-Tree"
        
        let buttonWrapper = ARRealityKitSwiftUIView(anchorTitle: "Selector-Tree-Background", desiredWidth: 0.15, desiredHeight: 0.105) {
            Text("Forest")
                .fontWeight(.medium)
                .padding(8)
        ...
        baseRectangleEntity.name = "Selector-Tree-Background"
        ...
        textEntity.name = "Selector-Tree-Text"
        selectorAnchor.addChild(textEntity)*/
        
        guard let pathName = Bundle.main.path(forResource: "Tree_Stage0_Level1", ofType: "usdz"),
        ...
        sampleEntity.name = "Selector-Tree-ModelSample"
